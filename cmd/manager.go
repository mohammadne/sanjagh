...
func NewManager(cfg *config.Config) *cobra.Command {
	...
	cmd.Flags().IntVar(&manager.metricsPort, "metrics-bind-port", 8080, "The port the metric endpoint binds to")
	cmd.Flags().IntVar(&manager.probePort, "health-probe-bind-port", 8081, "The port the probe endpoint binds to")
